...
function listarasistentes(){
    	$consulta = $this->db->query('select per.id IDPER,perage.TBPERSONASXTBAGENDAS,per.NUMERODOC, per.PNOMBRE,per.PAPELLIDO,age.NOMBRE_ACTIVIDAD from TBPERSONAS per 
...
function listarfactura(){
	    ...
	    $consulta = $this->db->query('select  ID, DESCRIPCION from TBCONCEPTOSFACTURAS');
	    if($consulta->num_rows()>0){
	        $a=0;
	        foreach ($consulta->result() as $fila){
